...
import { signInWithSSO } from '@/lib/auth/sso'
    ...
    <Card className="w-full max-w-md">
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="relative">
            <div className="absolute inset-0 flex items-center">
              <span className="w-full border-t border-[var(--color-border-divider)]" />
            </div>
            <div className="relative flex justify-center text-xs uppercase">
              <span className="bg-[var(--color-background-card)] px-2 text-[var(--color-text-muted)]">
                Or continue with
              </span>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-4">
          {error && (
            <div className="rounded-lg bg-[var(--color-error-bg)] p-3 text-sm text-[var(--color-error-text)] border border-[var(--color-error-border)]">
              {error}
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            />
            {errors.email && (
              <p className="text-sm text-[var(--color-error-text)]">{errors.email.message}</p>
            )}
          </div>

          <div className="space-y-2">
            <label htmlFor="password" className="text-sm font-medium text-[var(--color-text-label)]">
              Password
            </label>
            <Input
              id="password"
            ...
            />
            {errors.password && (
              <p className="text-sm text-[var(--color-error-text)]">{errors.password.message}</p>
            )}
          </div>

          <div className="space-y-2">
            <label htmlFor="confirmPassword" className="text-sm font-medium text-[var(--color-text-label)]">
              Confirm Password
            </label>
            <Input
              id="confirmPassword"
